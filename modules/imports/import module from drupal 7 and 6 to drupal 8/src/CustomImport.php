...
*/
  ...
  public function import_videos(){
    // Importar dates fi i inici

    $path = drupal_get_path('module', 'cridajson') . '/content_type_espectacle.txt';
    ...
    if(file_exists($path)) {
      ...
      foreach ($rows as $key => $position) {
        $contador ++;
        $nid = $position['nid'];
        $data_inici = $position['field_dates_espectacle_value'];
        $data_fi = $position['field_dates_espectacle_value2'];
        $node = Node::load($nid);

        $node->set('field_data_inici', $data_inici);
        $node->set('field_data_fi_espectacle', $data_fi);
        $node->save();
      }
    }
  }
